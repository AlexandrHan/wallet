...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  const TEAM_KEY = TEAM_NAME
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, '_')
    .replace(/^_+|_+$/g, '') || 'installation_team';

  const PAID_KEY = `salary_installers_${TEAM_KEY}_paid_projects`;
  const PAID_OPEN_KEY = `salary_installers_${TEAM_KEY}_paid_open`;
  ...
  async function loadRule() {
    ...
    if (!rule) {
      throw new Error(`Для ${TEAM_NAME || 'цієї бригади'} не налаштоване правило зарплатні`);
    }
    ...
    return `
      <div class="card project-card${cardClass}" data-project-card="${project.id}" style="margin-bottom:12px;">
        ...
        <div class="project-body">
          ...
          <button type="button" class="btn save project-paid-btn" data-project-paid="${project.id}" ${paid ? 'disabled' : ''}>
            Оплачено
          </button>
        </div>
      </div>
  ...
  async function loadProjects() {
    if (!TEAM_NAME) {
      root.innerHTML = `
        <div class="card">
          <div style="font-size:14px; opacity:.8; text-align:center;">Не вказана бригада</div>
        </div>
      `;
      return;
    }
    ...
    root.innerHTML = `
      <div class="card">
        <div style="font-size:14px; opacity:.8; text-align:center;">Завантаження проєктів...</div>
      </div>
    `;
    ...
    try {
        ...
        root.innerHTML = `
          <div class="card">
            <div style="font-size:14px; opacity:.8; text-align:center;">
              Для ${esc(TEAM_NAME)} зараз встановлена ставка, а не виробіток.
            </div>
          </div>
        `;
        return;
      ...
      allItems = (Array.isArray(projects) ? projects : [])
        .filter(project => String(project?.installation_team || '').trim().toLowerCase() === TEAM_NAME.toLowerCase())
        .map(project => {
  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      ${activeItems.length
        ...
        : '<div class="card" style="margin-bottom:12px;"><div style="font-size:14px; opacity:.8; text-align:center;">Активних неоплачених проєктів немає</div></div>'}
    `;
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (!projectToggle || target.closest('[data-project-paid]')) return;
